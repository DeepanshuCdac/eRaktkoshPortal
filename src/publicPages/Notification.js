...
const Notification = () => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await fetch(
          `${BaseUrl}/eraktkosh/notifications/ticker`
        );
        if (!response.ok) {
  ...
  const handleNotificationClick = async (notification, id) => {
    ...
    const topPosition = rect
      ? rect.bottom + scrollTop - (containerRect?.top + scrollTop)
      : 0;
  ...
  if (loading)
    return (
      <p className="gradient_style page-wrapper">Loading notifications...</p>
    );
  if (error)
    return (
      <p className="gradient_style page-wrapper" style={{ color: "red" }}>
        Error: {error}
      </p>
    );
          ...
          >
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        position: "relative",
                        zIndex:
                          selectedNotification?.id === notification.id ? 10 : 1,
                      }}
                      onClick={() =>
                        handleNotificationClick(notification, notification.id)
                      }
                    >
                      {index === 0 && (
                      ...
                      <div className="d-flex align-items-start h-100">
                        ...
                        <div className="w-100 h-100 d-flex flex-column justify-content-between">
                          <h5 className="notification_title mb-1">
                            {notification.title}
                          </h5>
            ...
            {selectedNotification && (
              <>
                <div
                  className={`overlay ${
                    isModalVisible ? "fade-in" : "fade-out"
                  }`}
                  onClick={closeModal}
                />
                <div
                  ...
                  {modalLoading ? (
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: modalContent.replace(/<\/?strong>/g, ""),
                      }}
                    />
                  )}
                </div>
