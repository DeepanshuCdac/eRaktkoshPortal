...
const Faqs = () => {
    ...
    const filteredFaqs = faqItems.filter(faq => {
        const matchesSearch = faq.faqQuestion.toLowerCase().includes(searchText.toLowerCase()) ||
            faq.faqAnswer.toLowerCase().includes(searchText.toLowerCase());

        const matchesFilter = activeFilter === 'All' || faq.faqType === activeFilter;
            ...
            try {
                ...
                if (Array.isArray(faqResponse.data)) {
                    const faqsWithType = faqResponse.data.map(faq => {
                        // faq filters...
                        if (faq.faqQuestion.toLowerCase().includes("blood availability") || faq.faqAnswer.toLowerCase().includes("general")) {
                ...
                // captcha api....
                const captchaResponse = await axios.post(`${BaseUrl}/eraktkosh/regenerateCaptcha`);
            ...
            <div className="body_wrapper py-3">
                <div className="container">
                    ...
                    <div className="mb-2 d-flex gap-2 flex-wrap">
                        <p className={`mb-0 filters ${activeFilter === 'All' ? 'active' : ''}`}
                            onClick={() => setActiveFilter('All')}> All </p>
                        <p className={`mb-0 filters ${activeFilter === 'General & Blood Availability' ? 'active' : ''}`}
                            ...
                            {filteredFaqs.length > 0 ? (
                                <Collapse
                                    ...
                                    items={filteredFaqs.map((faq, index) => ({
                                        ...
                                        children: <div dangerouslySetInnerHTML={{ __html: faq.faqAnswer }} />,
                                        style: panelStyle
                                    }))} />
                            ) : (
                                    ...
                                    {captchaImage && (
                                        <div className="d-flex align-items-center">
                                            <img
                                                ...
                                                alt="CAPTCHA"
                                                style={{ height: '38px' }} />
                                            <img style={{ cursor: 'pointer', width: '20px', height: '20px' }} className="" onClick={fetchCaptcha} src="assets/images/refresh.png" />
                                        </div>
                                    )}
                                    <Input
                                        placeholder="Captcha"
